 ...
 *
 * A seção inteira é UM painel de grafite, abertura incluída. A introdução era
 * clara e o resto escuro, o que abria um degrau de superfície no meio de um
 * assunto só; agora ela é a primeira baia do painel.
 *
 * Dentro do painel as baias se separam por fio, não por troca de cor — é a
 * gramática de instrumento que a página já usa. A variedade vem do arranjo
 * interno: a primeira é jogável em largura cheia, e as duas seguintes são
 * espelhos exatos uma da outra, com a captura trocando de lado.
 */
export function WhatYouPractice() {
  const { practice: s } = site;

  return (
    <section id="pratica" className="grain relative bg-graphite">
      <Container className="relative z-[2] py-[var(--space-surface)]">
        {/* ---------------------------------------------- abertura */}
        <SectionMark label={s.kicker} tone="dark" />
        ...
        <div className="mt-[var(--space-mark)] grid gap-x-12 gap-y-9 lg:grid-cols-12">
          ...
          <motion.p
            {...reveal(0.12)}
            className="max-w-[40ch] self-center text-lead leading-[1.5] text-white/60 lg:col-span-5 lg:col-start-8"
          >
            {s.intro}
          </motion.p>
        </div>
        ...
        {/*
          Índice das três capacidades: anuncia as baias que vêm em seguida.
          Sem números e sem réguas verticais, para não repetir a composição da
          seção 02.

          Distribuídos pela largura, não em terços iguais. Em colunas de 1/3 o
          texto é curto demais para preenchê-las, e sobravam ~200px mortos
          depois do terceiro item: a régua ia até a borda e o conteúdo parava
          antes, jogando o conjunto para a esquerda. Com justify-between o
          primeiro item encosta na margem esquerda e o terceiro na direita.
        */}
        <ul className="mt-16 flex flex-col gap-y-10 border-t border-white/10 py-14 sm:mt-20 sm:flex-row sm:justify-between sm:gap-x-12">
          {s.capabilities.map((c, i) => (
            <motion.li key={c.term} {...reveal(0.18 + i * 0.08)} className="sm:max-w-[34ch]">
              <h3 className="text-[clamp(1.25rem,2vw,1.5rem)] font-semibold tracking-[-0.025em] text-bone">
                {c.term}
              </h3>
              <p className="mt-2.5 max-w-[30ch] text-[0.9375rem] leading-[1.55] text-white/50">
                {c.body}
              </p>
            </motion.li>
          ))}
...
/**
 * Separação entre baias: fio e respiro igual dos dois lados.
 *
 * `flush` dispensa o respiro de cima, para quando o bloco anterior já pagou
 * o dele. É o caso do índice das capacidades: ele fecha com py-14 e a régua
 * desta baia entra logo em seguida, senão os três itens ficam grudados na
 * régua de cima com todo o vazio embaixo.
 */
function Slab({ children, flush = false }: { children: React.ReactNode; flush?: boolean }) {
  return (
    <div
      className={`border-t border-white/10 pt-[var(--space-block)] ${
        flush ? "" : "mt-[var(--space-block)]"
      }`}
    >
      {children}
    </div>
...
/**
 * Baia de captura: duas colunas na mesma régua, 7 para a imagem e 5 para o
 * texto, centradas verticalmente.
 *
